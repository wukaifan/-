// 站点内容集中在这里，后续替换图片 / 更新数据都改这个文件即可。

import detail01 from "../assets/work/detail-01.jpg";
import detail02 from "../assets/work/detail-02.jpg";
import detail03 from "../assets/work/detail-03.jpg";
import detail04 from "../assets/work/detail-04.jpg";
import detail05 from "../assets/work/detail-05.jpg";
import caseRoseSerum from "../assets/work/case-rose-serum.jpg";
import caseDiyBucket from "../assets/work/case-diy-bucket.jpg";
import caseCrayons from "../assets/work/case-crayons.jpg";
import caseMeshBag from "../assets/work/case-mesh-bag.jpg";
import caseLinedPens from "../assets/work/case-lined-pens.jpg";
import caseRoseSerumThumb from "../assets/work/case-rose-serum-thumb.jpg";
import caseDiyBucketThumb from "../assets/work/case-diy-bucket-thumb.jpg";
import caseCrayonsThumb from "../assets/work/case-crayons-thumb.jpg";
import caseMeshBagThumb from "../assets/work/case-mesh-bag-thumb.jpg";
import caseLinedPensThumb from "../assets/work/case-lined-pens-thumb.jpg";
import caseBabyCare from "../assets/work/case-baby-care.jpg";
import caseBabyCareThumb from "../assets/work/case-baby-care-thumb.jpg";
import caseMulti01 from "../assets/work/case-multi-01.jpg";
import caseMulti02 from "../assets/work/case-multi-02.jpg";
import caseMulti03 from "../assets/work/case-multi-03.jpg";
import caseMulti04 from "../assets/work/case-multi-04.jpg";
import caseMulti05 from "../assets/work/case-multi-05.jpg";
import caseMulti06 from "../assets/work/case-multi-06.jpg";
import caseMulti07 from "../assets/work/case-multi-07.jpg";
import caseMulti08 from "../assets/work/case-multi-08.jpg";
import caseMulti01Thumb from "../assets/work/case-multi-01-thumb.jpg";
import caseMulti02Thumb from "../assets/work/case-multi-02-thumb.jpg";
import caseMulti03Thumb from "../assets/work/case-multi-03-thumb.jpg";
import caseMulti04Thumb from "../assets/work/case-multi-04-thumb.jpg";
import caseMulti05Thumb from "../assets/work/case-multi-05-thumb.jpg";
import caseMulti06Thumb from "../assets/work/case-multi-06-thumb.jpg";
import caseMulti07Thumb from "../assets/work/case-multi-07-thumb.jpg";
import caseMulti08Thumb from "../assets/work/case-multi-08-thumb.jpg";
import posterCampus from "../assets/work/poster-campus.jpg";
import posterTouch17 from "../assets/work/poster-touch17.png";
import posterAnniv from "../assets/work/poster-anniv.jpg";
import posterReward from "../assets/work/poster-reward.jpg";
import posterUplive from "../assets/work/poster-uplive.jpg";
import posterDouyin from "../assets/work/poster-douyin.jpg";
import posterCapybara from "../assets/work/poster-capybara.jpg";
import posterDesert from "../assets/work/poster-desert.jpg";
import posterDeepsea from "../assets/work/poster-deepsea.jpg";
import posterIsland from "../assets/work/poster-island.jpg";
import posterCampusThumb from "../assets/work/poster-campus-thumb.jpg";
import posterTouch17Thumb from "../assets/work/poster-touch17-thumb.jpg";
import posterAnnivThumb from "../assets/work/poster-anniv-thumb.jpg";
import posterRewardThumb from "../assets/work/poster-reward-thumb.jpg";
import posterUpliveThumb from "../assets/work/poster-uplive-thumb.jpg";
import posterDouyinThumb from "../assets/work/poster-douyin-thumb.jpg";
import posterCapybaraThumb from "../assets/work/poster-capybara-thumb.jpg";
import posterDesertThumb from "../assets/work/poster-desert-thumb.jpg";
import posterDeepseaThumb from "../assets/work/poster-deepsea-thumb.jpg";
import posterIslandThumb from "../assets/work/poster-island-thumb.jpg";
import posterDouble11 from "../assets/work/poster-double11.jpg";
import posterDouble11Thumb from "../assets/work/poster-double11-thumb.jpg";
import kvWomenDay from "../assets/work/kv-women-day.jpg";
import kv618Skincare from "../assets/work/kv-618-skincare.jpg";
import kvChildrenDay from "../assets/work/kv-children-day.jpg";
import kvDouble11New from "../assets/work/kv-double11-new.jpg";
import kv618Carnival from "../assets/work/kv-618-carnival.jpg";
import kv618Touchmark from "../assets/work/kv-618-touchmark.jpg";
import kvWomenDayThumb from "../assets/work/kv-women-day-thumb.jpg";
import kv618SkincareThumb from "../assets/work/kv-618-skincare-thumb.jpg";
import kvChildrenDayThumb from "../assets/work/kv-children-day-thumb.jpg";
import kvDouble11NewThumb from "../assets/work/kv-double11-new-thumb.jpg";
import kv618CarnivalThumb from "../assets/work/kv-618-carnival-thumb.jpg";
import kv618TouchmarkThumb from "../assets/work/kv-618-touchmark-thumb.jpg";
import otherAnniv from "../assets/work/other-anniv.png";
import otherRecruit from "../assets/work/other-recruit.jpg";
import otherNewyear from "../assets/work/other-newyear.png";
import otherStudy from "../assets/work/other-study.jpg";
import otherSpring from "../assets/work/other-spring.png";
import otherSpringfestival from "../assets/work/other-springfestival.png";
import otherChildren61 from "../assets/work/other-children61.png";
import otherAnnivThumb from "../assets/work/other-anniv-thumb.jpg";
import otherRecruitThumb from "../assets/work/other-recruit-thumb.jpg";
import otherNewyearThumb from "../assets/work/other-newyear-thumb.jpg";
import otherStudyThumb from "../assets/work/other-study-thumb.jpg";
import otherSpringThumb from "../assets/work/other-spring-thumb.jpg";
import otherSpringfestivalThumb from "../assets/work/other-springfestival-thumb.jpg";
import otherChildren61Thumb from "../assets/work/other-children61-thumb.jpg";

export const profile = {
  name: "吴凯帆",
  nameEN: "WU KAIFAN",
  role: "视觉设计师 / AI设计师 / 电商设计师",
  roleShort: "VISUAL · AI · E-COMMERCE DESIGNER",
  tagline: "让每一次浏览，都成为一次值得记住的设计体验。",
  sub: "六年电商与视觉设计经验，习惯从商业目标出发做设计。不止于好看，更在意承载品牌气质、提升转化、优化体验。",
  bio: [
    "我是吴凯帆，一名专注转化与体验的视觉设计师。",
    "过去六年，我长期服务于电商与品牌项目，主导主图详情页、店铺装修、活动专题与创意海报的视觉落地。我擅长把复杂的商品语言翻译成一眼能懂的视觉叙事，让设计真正为生意服务。",
    "同时我持续探索 AI 辅助设计工作流，用生成式工具放大创意效率，把更多时间留给判断与审美。",
  ],
  age: 27,
  phone: "150 8355 7157",
  phoneRaw: "15083557157",
  email: "2847642883@qq.com",
  wechat: "wu091909",
  location: "江西 · 南昌",
  status: "可合作 / 开放项目沟通",
};

export const nav = [
  { id: "home", label: "首页", index: "01" },
  { id: "about", label: "关于", index: "02" },
  { id: "work", label: "项目", index: "03" },
  { id: "skills", label: "能力", index: "04" },
  { id: "contact", label: "联系", index: "05" },
];

export const stats = [
  { to: 6, unit: "+", label: "视觉 & 电商设计经验" },
  { to: 500, unit: "+", label: "项目落地交付" },
  { to: 30, unit: "+", label: "服务品牌与店铺" },
  { to: 92, unit: "%", label: "客户复购与满意度" },
];

export const education = {
  school: "江西省电子信息工程学校",
  period: "2015.09 — 2018.07",
  major: "平面设计",
  courses: "平面设计基础 · 色彩原理 · 造型基础 · 设计史 · 美术史",
};

export const experience = [
  {
    period: "2025.09 — 至今",
    company: "江西书涯轩文化科技有限公司",
    role: "电商设计",
    tags: ["主图详情页", "抖店直播贴片", "活动专题", "线下物料"],
    points: [
      "负责主图详情页制作与优化、抖店直播贴片、KT板制作、直播背景与店铺装修。",
      "活动专题页设计，日常页面维护美化，设计与制作电商平台的创意海报及活动宣传。",
      "负责公司线下物料宣传海报、推广图及周年庆活动视觉。",
    ],
  },
  {
    period: "2024.05 — 2025.05",
    company: "辰上商贸有限公司",
    role: "平面设计",
    tags: ["主图详情页", "店铺装修", "产品渲染"],
    points: [
      "负责主图详情页制作、店铺装修与产品渲染，统一线上视觉风格。",
      "日常页面维护美化，上传活动产品主图，定期清理滞销品视觉素材。",
    ],
  },
  {
    period: "2019.02 — 2022.12",
    company: "麦匠科技有限公司",
    role: "电商设计",
    tags: ["活动专题", "创意海报", "钻展直通车"],
    points: [
      "负责主图详情页、钻展直通车与活动专题页设计，输出电商平台创意海报。",
      "设计与制作活动宣传、产品推广图，完成商品后期修图与视觉美化。",
    ],
  },
];

// 精选项目。cover 目前用渐变 + 文字构成"可视化封面"，后续直接替换成作品截图即可。
export const projects = [
  {
    title: "主图详情页 · 转化引擎",
    brand: "辰上商贸",
    category: "电商视觉",
    year: "2026",
    tags: ["主图设计", "详情页", "场景合成"],
    desc: "以卖点为线索重组图片语言，用场景化合成提升停留与转化，让用户在 3 秒内理解产品价值。",
    cover: "ember",
    featured: true,
    link: "#main-detail",
    coverImage: caseRoseSerumThumb,
    shots: [detail01, detail02, detail03, detail04, detail05],
  },
  {
    title: "创意海报 · 主题表达",
    brand: "综合 / 活动",
    category: "海报设计",
    year: "2026",
    tags: ["创意海报", "活动宣传", "视觉传达"],
    desc: "以主题为核做海报视觉表达，在信息与情绪之间找到平衡，让活动在传播与线下都足够吸睛、耐看。",
    cover: "violet",
    link: "#posters",
    coverImage: posterDeepseaThumb,
  },
  {
    title: "首页视觉 · 大促主视觉",
    brand: "综合 · 电商",
    category: "首页视觉",
    year: "2026",
    tags: ["首页KV", "主视觉", "大促"],
    desc: "为店铺首页打造的大促主视觉，把活动氛围与品牌调性统一到首屏，第一时间抓住浏览者注意力。",
    cover: "ocean",
    link: "#kv",
    coverImage: kv618TouchmarkThumb,
  },
  {
    title: "其他设计 · 多元落地",
    brand: "综合",
    category: "其他设计",
    year: "2026",
    tags: ["KT板", "线下物料", "包装贴纸"],
    desc: "覆盖 KT 板、直播背景、线下物料、包装贴纸等多元设计，统一品牌调性，让每个触点都保持质感。",
    cover: "lime",
    link: "#others",
    coverImage: otherAnnivThumb,
  },
];

// 主图详情页 · 二级页内容
export const mainDetailArt = [
  {
    title: "Botanée 玫瑰焕颜精华液",
    brand: "Botanée",
    category: "电商视觉 · 护肤",
    year: "2026",
    tags: ["主图设计", "详情页", "护肤视觉"],
    desc: "以「润」贯穿成分到功效，用柔和的玫瑰色谱与留白，塑造高端水乳的质感与信任感。",
    img: caseRoseSerum,
    thumb: caseRoseSerumThumb,
  },
  {
    title: "DIY 洞洞笔筒 · 卡皮巴拉联名",
    brand: "卡皮巴拉联名",
    category: "电商视觉 · 文创",
    year: "2026",
    tags: ["主图设计", "详情页", "IP联名"],
    desc: "萌趣联名 IP + 环保护眼卖点分层，用明亮场景色放大儿童文具的吸引力与购买欲。",
    img: caseDiyBucket,
    thumb: caseDiyBucketThumb,
  },
  {
    title: "易可洗的旋转蜡笔",
    brand: "ToucMarkers",
    category: "电商视觉 · 文具",
    year: "2026",
    tags: ["主图设计", "详情页", "儿童用品"],
    desc: "把「不脏手」「易清洗」的痛点与专业检测放在最前，建立放心、好用的儿童绘画体验。",
    img: caseCrayons,
    thumb: caseCrayonsThumb,
  },
  {
    title: "纳集系列 A3 网纱美术袋",
    brand: "纳集",
    category: "电商视觉 · 收纳",
    year: "2026",
    tags: ["主图设计", "详情页", "场景合成"],
    desc: "温暖橙色调 + 大容量分层收纳的画面，把「装得下、找得到」的实用卖点讲得清楚耐看。",
    img: caseMeshBag,
    thumb: caseMeshBagThumb,
  },
  {
    title: "儿童双头勾线笔",
    brand: "ToucMarkers",
    category: "电商视觉 · 文具",
    year: "2026",
    tags: ["主图设计", "详情页", "卖点可视化"],
    desc: "多场景手绘示范 + 安全环保认证，强化「好写、干净、安心」的品牌承诺与使用体验。",
    img: caseLinedPens,
    thumb: caseLinedPensThumb,
  },
  {
    title: "CareBaby 婴儿保湿乳 · 温和守护",
    brand: "CareBaby",
    category: "电商视觉 · 母婴",
    year: "2026",
    tags: ["主图设计", "详情页", "母婴视觉"],
    desc: "以「温和守护」为主线，从成分、功效到使用场景逐层展开，建立 0-3 岁宝妈的信赖与安全感。",
    img: caseBabyCare,
    thumb: caseBabyCareThumb,
  },
  {
    title: "软头透彩丙烯马克笔",
    brand: "TouchMark 泰驰马克",
    category: "电商视觉 · 文具",
    year: "2026",
    tags: ["主图设计", "详情页", "儿童文具"],
    desc: "软头透彩 + 老师推荐，用明亮场景放大马克笔的色彩表现与儿童友好卖点。",
    img: caseMulti01,
    thumb: caseMulti01Thumb,
  },
  {
    title: "易水洗儿童白板笔",
    brand: "TouchMark 泰驰马克",
    category: "电商视觉 · 文具",
    year: "2026",
    tags: ["主图设计", "详情页", "儿童用品"],
    desc: "把「可水洗」「色彩鲜艳易涂易擦」卖点前置，建立家长放心、孩子好用的白板笔体验。",
    img: caseMulti02,
    thumb: caseMulti02Thumb,
  },
  {
    title: "GAO DONG 蜂蜡润唇膏",
    brand: "GAO DONG",
    category: "电商视觉 · 个护",
    year: "2026",
    tags: ["主图设计", "详情页", "个护视觉"],
    desc: "30s 速淡纹、保湿 3 天不干燥，天然蜂蜡维 E 滋养，用清新绿调传递温和修护。",
    img: caseMulti03,
    thumb: caseMulti03Thumb,
  },
  {
    title: "RUNMUI 氨基酸净颜洗面奶",
    brand: "RUNMUI",
    category: "电商视觉 · 个护",
    year: "2026",
    tags: ["主图设计", "详情页", "个护视觉"],
    desc: "云朵绵柔泡、轻柔净毛孔，突出「不泛红不紧绷不假滑」的氨基酸温和清洁体验。",
    img: caseMulti04,
    thumb: caseMulti04Thumb,
  },
  {
    title: "明谦 MQ COFFEE 深烘焙咖啡礼盒",
    brand: "明谦 MQ COFFEE",
    category: "电商视觉 · 食品",
    year: "2026",
    tags: ["主图设计", "详情页", "食品视觉"],
    desc: "天猫 618 大促主图，「深烘焙油脂王」用浓郁油脂与立减优惠强化购买理由。",
    img: caseMulti05,
    thumb: caseMulti05Thumb,
  },
  {
    title: "韶音 OPENFIT T910 开放式耳机",
    brand: "SHOKZ 韶音",
    category: "电商视觉 · 数码",
    year: "2026",
    tags: ["主图设计", "详情页", "数码视觉"],
    desc: "开放式运动耳机大促主图，把「久戴不胀痛、行动不掉落、开放不漏音」与直降 200 讲清楚。",
    img: caseMulti06,
    thumb: caseMulti06Thumb,
  },
  {
    title: "抱抱桶软头丙烯马克笔",
    brand: "TouchMark 泰驰马克",
    category: "电商视觉 · 文具",
    year: "2026",
    tags: ["主图设计", "详情页", "多材质"],
    desc: "抱抱桶装丙烯马克笔，「铅笔、石头、木材、布料都可绘画」，突出多材质创作。",
    img: caseMulti07,
    thumb: caseMulti07Thumb,
  },
  {
    title: "高弹软头丙烯马克笔",
    brand: "TouchMark 泰驰马克",
    category: "电商视觉 · 文具",
    year: "2026",
    tags: ["主图设计", "详情页", "儿童文具"],
    desc: "「覆盖力强、高弹软头、万物可画」卖点分层，用高饱和色彩放大产品表现力。",
    img: caseMulti08,
    thumb: caseMulti08Thumb,
  },
];

// 创意海报 · 二级页内容
export const posterArt = [
  {
    title: "TouchMark 17 周年庆 · 感恩相伴",
    brand: "TouchMark",
    category: "品牌海报 · 周年",
    year: "2026",
    tags: ["周年庆", "活动海报", "促销"],
    desc: "以周年庆为主线的活动海报，把感谢与福利一起讲清，色彩明亮、节奏欢快。",
    img: posterAnniv,
    thumb: posterAnnivThumb,
  },
  {
    title: "博主合作奖励制度",
    brand: "TouchMark",
    category: "品牌海报 · 规则",
    year: "2026",
    tags: ["合作激励", "规则说明", "达人"],
    desc: "把复杂的合作奖励规则梳理成清晰层级，信息密度高但不失秩序。",
    img: posterReward,
    thumb: posterRewardThumb,
  },
  {
    title: "买家秀征集令 · 丰厚奖品",
    brand: "TouchMark",
    category: "活动海报 · 征集",
    year: "2026",
    tags: ["买家秀", "征集", "奖品"],
    desc: "用醒目的奖品分层与参与要求，调动用户分享欲，活动氛围热烈。",
    img: posterUplive,
    thumb: posterUpliveThumb,
  },
  {
    title: "买家秀征集令 · 敢晒敢送",
    brand: "TouchMark",
    category: "活动海报 · 征集",
    year: "2026",
    tags: ["买家秀", "征集", "抖音"],
    desc: "针对社交平台传播的征集海报，强视觉 + 清晰规则，引导到店参与。",
    img: posterDouyin,
    thumb: posterDouyinThumb,
  },
  {
    title: "卡皮巴拉买家秀征集令",
    brand: "TouchMark",
    category: "活动海报 · 征集",
    year: "2026",
    tags: ["征集", "卡皮巴拉", "营销"],
    desc: "以 IP 形象拉近用户距离，奖品与参与要求一目了然，亲和且吸睛。",
    img: posterCapybara,
    thumb: posterCapybaraThumb,
  },
  {
    title: "沙漠越野记 · 户外满减",
    brand: "综合 · 户外",
    category: "AI 视觉 · 促销",
    year: "2026",
    tags: ["沙漠", "3D 视觉", "满减"],
    desc: "用 3D 场景渲染的促销主视觉，把「满 320 减 110」放进有想象力的画面。",
    img: posterDesert,
    thumb: posterDesertThumb,
  },
  {
    title: "深海逐梦 · 自在遨游",
    brand: "综合 · 主题",
    category: "AI 视觉 · 主题",
    year: "2026",
    tags: ["深海", "3D 视觉", "主题"],
    desc: "以海洋童趣为主题的画面，用色彩与角色营造沉浸式的梦幻氛围。",
    img: posterDeepsea,
    thumb: posterDeepseaThumb,
  },
  {
    title: "海岛拾贝季 · 生鲜满减",
    brand: "综合 · 生鲜",
    category: "AI 视觉 · 促销",
    year: "2026",
    tags: ["海岛", "3D 视觉", "满减"],
    desc: "清透的海岛场景 + 醒目折扣信息，把生鲜好物的夏日氛围拉满。",
    img: posterIsland,
    thumb: posterIslandThumb,
  },
  {
    title: "TouchMark 2026 校园招聘",
    brand: "TouchMark",
    category: "招聘海报",
    year: "2026",
    tags: ["校园招聘", "品牌", "海报"],
    desc: "青春活力的招聘主视觉，把「与瓴德同行 · 让未来闪光」的愿景说给年轻人。",
    img: posterCampus,
    thumb: posterCampusThumb,
  },
  {
    title: "共创美好 · 拾光而行",
    brand: "TouchMark",
    category: "品牌海报 · 文化",
    year: "2026",
    tags: ["企业文化", "周年", "品牌"],
    desc: "企业价值观主题海报，用品牌色与愿景语传达共创与向上的团队精神。",
    img: posterTouch17,
    thumb: posterTouch17Thumb,
  },
  {
    title: "双十一 · 放肆购",
    brand: "TouchMark",
    category: "活动海报 · 大促",
    year: "2026",
    tags: ["双十一", "3D 视觉", "大促"],
    desc: "以 3D 场景渲染的双十一大促主视觉，把「超值好物 一站购齐」用欢快氛围与醒目标题讲透。",
    img: posterDouble11,
    thumb: posterDouble11Thumb,
  },
];

// 首页视觉 · 二级页内容
export const homeKvArt = [
  {
    title: "38 女神节 · 为爱焕新",
    brand: "木子田",
    category: "首页视觉 · 节日",
    year: "2026",
    tags: ["38 节", "3D 视觉", "个护"],
    desc: "以紫粉色星空与礼盒营造节日仪式感，把「为爱焕新」的表达做出高级与温柔。",
    img: kvWomenDay,
    thumb: kvWomenDayThumb,
  },
  {
    title: "618 狂欢购 · 乐享好礼",
    brand: "CHENCTICN",
    category: "首页视觉 · 大促",
    year: "2026",
    tags: ["618", "3D 视觉", "护肤"],
    desc: "用热烈的红色调与礼赠元素，把「狂欢购 · 乐享好礼」的大促氛围拉满。",
    img: kv618Skincare,
    thumb: kv618SkincareThumb,
  },
  {
    title: "六一童趣季 · 绘画乐其中",
    brand: "综合",
    category: "首页视觉 · 节日",
    year: "2026",
    tags: ["六一", "3D 视觉", "童趣"],
    desc: "童趣六一的主题视觉，用软萌的角色与明快色彩，营造轻松开心的节日气氛。",
    img: kvChildrenDay,
    thumb: kvChildrenDayThumb,
  },
  {
    title: "双11 新品狂欢购",
    brand: "TouchMark",
    category: "首页视觉 · 大促",
    year: "2026",
    tags: ["双11", "3D 视觉", "文具"],
    desc: "双11 新品首发主视觉，以「满 300 减 150」的强利益点与城市场景制造热度。",
    img: kvDouble11New,
    thumb: kvDouble11NewThumb,
  },
  {
    title: "618 与你狂欢 · 年中盛典",
    brand: "智可优",
    category: "首页视觉 · 大促",
    year: "2026",
    tags: ["618", "3D 视觉", "母婴"],
    desc: "以 618 年中盛典为主题的主视觉，把「全场优惠 5 折起」放进热闹欢快的亲子购物场景，突出母婴好物的温度。",
    img: kv618Carnival,
    thumb: kv618CarnivalThumb,
  },
  {
    title: "618 夏日大促 · 清凉狂欢",
    brand: "TouchMark",
    category: "首页视觉 · 大促",
    year: "2026",
    tags: ["618", "3D 视觉", "画材"],
    desc: "以夏日水乐园为场景的品牌大促主视觉，把「全场满 300 减 50、爆款 5 折起」融进缤纷画材与清凉氛围。",
    img: kv618Touchmark,
    thumb: kv618TouchmarkThumb,
  },
];

// 其他设计 · 二级页内容
export const otherArt = [
  {
    title: "TouchMark 17 周年盛典 · 爆款 1 折起",
    brand: "TouchMark",
    category: "其他设计 · 品牌",
    year: "2026",
    tags: ["周年盛典", "3D 视觉", "品牌"],
    desc: "以热烈橙色与飘带礼盒营造盛典氛围，把「爆款 1 折起」的重量级福利讲得响亮。",
    img: otherAnniv,
    thumb: otherAnnivThumb,
  },
  {
    title: "春季招聘 · 职等你来",
    brand: "TouchMark",
    category: "其他设计 · 招聘",
    year: "2026",
    tags: ["招聘海报", "校招", "信息设计"],
    desc: "把招聘岗位与职责做清晰的信息层级，用明快色调让「职等你来」更有人气。",
    img: otherRecruit,
    thumb: otherRecruitThumb,
  },
  {
    title: "年货狂欢不打烊",
    brand: "TouchMark",
    category: "其他设计 · 节日",
    year: "2026",
    tags: ["年货节", "3D 视觉", "电商"],
    desc: "用灯笼、福字与暖红底色做年货节主视觉，热闹且年味十足。",
    img: otherNewyear,
    thumb: otherNewyearThumb,
  },
  {
    title: "学霸养成进行时 · 春季招生",
    brand: "综合",
    category: "其他设计 · 教育",
    year: "2026",
    tags: ["招生", "教育", "海报"],
    desc: "把课程保障与春季特惠做信息分层，用亲和插画拉近家长与学生的距离。",
    img: otherStudy,
    thumb: otherStudyThumb,
  },
  {
    title: "春风绘色 · 笔尖生花",
    brand: "TouchMark",
    category: "其他设计 · 主题",
    year: "2026",
    tags: ["春季", "3D 视觉", "主题"],
    desc: "以清新春日与露营元素做主题视觉，把「笔尖生花」的诗意表达出来。",
    img: otherSpring,
    thumb: otherSpringThumb,
  },
  {
    title: "春节不打烊 · 物流不停",
    brand: "TouchMark",
    category: "其他设计 · 节日",
    year: "2026",
    tags: ["春节", "3D 视觉", "电商"],
    desc: "用红金配色的春节主视觉，传达「物流不停歇、好礼送到家」的决心与氛围。",
    img: otherSpringfestival,
    thumb: otherSpringfestivalThumb,
  },
  {
    title: "童心未泯 · 快乐 61",
    brand: "TouchMark",
    category: "其他设计 · 节日",
    year: "2026",
    tags: ["六一", "3D 视觉", "童趣"],
    desc: "用清透海景与软萌角色打造六一主题，色调轻快、充满童真快乐。",
    img: otherChildren61,
    thumb: otherChildren61Thumb,
  },
];

export const skills = [
  {
    index: "01",
    title: "视觉设计",
    en: "Visual Design",
    level: "精通",
    desc: "色彩、版式、字体与审美判断。能在理性约束下做出有情绪、有记忆点的视觉。",
  },
  {
    index: "02",
    title: "电商设计",
    en: "E-commerce",
    level: "精通",
    desc: "主图、详情页、活动专题、钻展直通车，覆盖从浏览到转化的完整链路。",
  },
  {
    index: "03",
    title: "AI 辅助设计",
    en: "AI Workflow",
    level: "熟练",
    desc: "把生成式工具接入工作流，快速探索方向、合成素材，把创意效率放大。",
  },
];

export const tools = [
  { name: "Photoshop", short: "Ps", level: 95 },
  { name: "Illustrator", short: "Ai", level: 88 },
  { name: "AI 生成工具", short: "Aï", level: 85 },
];
